$(function() {
	...
	createTabledate(parseInt(withID("aboluo-yearSelect").value),
			parseInt(withID("aboluo-selectmonth").value));
	// 上月下月的a标签给事件
	leftrightclick();
	// 设置右边显示栏显示内容，显示栏还可以设置节假日的状态等
	setRigth(new Date().getFullYear(), new Date().getMonth() + 1, new Date()
			.getDate());
	paging();
});
// 阻止冒泡
...
function createSelectYear() {
	...
	for (var i = 0; i <= 5; i++) {
		yearSelect.options.add(new Option((i + currYear) + "年", i + currYear));
	...
	yearSelect.onchange = function(e) {
		var evt = e ? e : event;
		var yearVal = evt.srcElement ? evt.srcElement.value : evt.target.value;
		paging(yearVal);
		...
		createTabledate(withID("aboluo-yearSelect").value,
		...
		if (aclick == "") {
			...
			if (new Date().getDate() > pervdays1) {
				setRigth(withID("aboluo-yearSelect").value,
						withID("aboluo-selectmonth").value, pervdays1);
			} else {
				setRigth(withID("aboluo-yearSelect").value,
						withID("aboluo-selectmonth").value, new Date()
								.getDate());
			}
		} else {
			...
			setRigth(withID("aboluo-yearSelect").value,
					withID("aboluo-selectmonth").value, aarr[2]);
		}
...
function getjjrszModelByYear(year) {
	...
	var httpService = new HTTPService(
			"holidayMaintain/queryHolidayByYear.ajax", hyear,
			function(data) {
				holidayList = data;
				setA();
			});
	httpService.sendJSON();
}
...
function createTabledate(year, yue) {
	...
	if (rilitabledele != "" && rilitabledele != null
	...
	for (var i = lastdays; i <= pervMonthlastDay; i++) {
		...
		var a = getA(parseInt(yue) - 1 == 0 ? parseInt(year) - 1 : year,
				parseInt(yue) - 1 == 0 ? 12 : parseInt(yue) - 1, i);
		a.style.color = "#BFBFC5";
		// a.href
		// ='javascript:pervA('+parseInt(yue)-1==0?parseInt(year)-1:year+','+parseInt(yue)-1==0?12:parseInt(yue)-1+','+i+');';
		td.appendChild(a);
	...
	// 当月除开第一行的起点
	var currmonthStartDay = currMonthLashDay - (currMonthLashDay - startDays)
			+ 1;
	// 当月还剩余的天数
	...
	for (var i = 0; i < xhcs; i++) {
		...
		for (var n = 1; n <= 7; n++) {
			...
			if (startDays == 0) {
				var c = getA(
						parseInt(yue) + 1 == parseInt(13) ? parseInt(year) + 1
								: year, parseInt(yue) + 1 == parseInt(13) ? 1
								: parseInt(yue) + 1, jilvn);
	...
	for (var i = 0; i < rows.length; i++) {
		for (var j = 0; j < rows[i].cells.length; j++) {
			...
			if (j >= rows[i].cells.length - 2) {
				if (cell.getAttribute("class") != "aboluo-nextMonthDays"
						&& cell.getAttribute("class") != "aboluo-pervMonthDays") {
					a.style.color = "red";
				}
			}
		}
...
function setRigth(year, yue, day) {
	...
	var span6 = newElement('span');
	span6.innerHTML = "&nbsp;设置日期类型:";
	szrq.appendChild(span6);
	
	var szrqselect = newElement("select");
	szrqselect.id="dayType";
	...
	if (!bool) {
		szrqselect.options.add(new Option("节假日", "1"));
		szrqselect.options.add(new Option("休息日", "2"));
	} else {
		szrqselect.options.add(new Option("节假日", "1"));
		szrqselect.options.add(new Option("工作日", "3"));
	}
	szrq.appendChild(szrqselect);
	
	var span2 = newElement('span');
	span2.innerHTML = "&nbsp;设置节假日名称:";
	szrq.appendChild(span2);
	var holiday = newElement('input');
	holiday.type = "text";
	holiday.name="holiday";
	holiday.style.width = (withClass("aboluo-rightdiv").offsetWidth * 0.9) + "px";
	szrq.appendChild(holiday);
	//设置开始时间
	var span3 = newElement('span');
	span3.innerHTML = "&nbsp;开始时间(1-24点):";
	szrq.appendChild(span3);
	var begin = newElement('input');
	begin.type = "text";
	begin.name="begin";
	begin.style.width = (withClass("aboluo-rightdiv").offsetWidth * 0.9) + "px";
	szrq.appendChild(begin);
	//设置结束时间
	var span4 = newElement('span');
	span4.innerHTML = "&nbsp;结束时间(1-24点):";
	szrq.appendChild(span4);
	var end = newElement('input');
	end.type = "text";
	end.name="end"
	end.style.width = (withClass("aboluo-rightdiv").offsetWidth * 0.9) + "px";
	szrq.appendChild(end);
	
	var szrqbutton = newElement('input');
	szrqbutton.type = "button";
	szrqbutton.style.backgroundColor = "#06cef9";
	...
	var span5 = newElement('span');
	span5.innerHTML = "&nbsp;&nbsp;&nbsp;";
	szrq.appendChild(span5);
	
	var delbutton = newElement('input');
	delbutton.id="delete";
	delbutton.type = "button";
	delbutton.style.backgroundColor = "#06cef9";
	delbutton.className = "btn"; // 设置class
	delbutton.value = "删除";
	delbutton.setAttribute("onclick", "javascript:delHoliday();");
	szrq.appendChild(delbutton);
	setaclass(year, yue, day);
...
function formatByYearyueday(year, yue, day) {
	...
	yue = yue.toString();
	day = day.toString();
	return year + "-" + (yue.length < 2 ? '0' + yue : yue) + "-"
...
function formatByDate(date) {
	...
	return daxx[0] + "-" + (daxx[1].length < 2 ? '0' + daxx[1] : daxx[1]) + "-"
			+ (daxx[2].length < 2 ? '0' + daxx[2] : daxx[2]);
}
...
function setA() {
	...
	for (var i = 0; i < arr.length; i++) {
		...
		if (arr[i].parentNode.className == "aboluo-pervMonthDays") {
			arr[i].setAttribute("onclick", "javascript:pervA(" + datearr[0]
					+ "," + datearr[1] + "," + datearr[2]
					+ ",this);javascript:stopBubble(this);")
		} else if (arr[i].parentNode.className == "aboluo-nextMonthDays") {
			arr[i].setAttribute("onclick", "javascript:nextA(" + datearr[0]
					+ "," + datearr[1] + "," + datearr[2]
					+ ",this);javascript:stopBubble(this);")
		} else {
			arr[i].setAttribute("onclick", "javascript:setRigth(" + datearr[0]
					+ "," + datearr[1] + "," + datearr[2]
					+ ");javascript:stopBubble(this);");
		}
		for(var n = 0; n < holidayList.length; n++){
			if(holidayList[n].commonDate.substring(0,10) == formatByDate(date)){
				var span = newElement('span');
				span.setAttribute("class", "aboluo-td-a-ban");
				span.setAttribute("style","text-align: center");
				//arr[i].style.background = "#ffffff";
				span.innerHTML = holidayList[n].festivalName;
...
function setB() {
	...
	for (var i = 0; i < arr.length; i++) {
		...
		for(var m = 0; m < lunarList.length; m++){
			if(lunarList[m].commonDate == formatByDate(date)){
				var span = newElement('span');
				span.setAttribute("class", "aboluo-td-a-xiu");
				//arr[i].style.background = "#ffffff";
				span.innerHTML = lunarList[m].lunarCalendar.substring(7,13);
				arr[i].appendChild(span);
			}
		}
	}
...
/*function setHolidayName() {
	...
	for (var i = 0; i < arr.length; i++) {
		var date = arr[i].getAttribute("date");
		for(var n = 0; n < holidayList.length; n++){
			if(holidayList[n].holidayTime.substring(0,10) == formatByDate(date)){
				var span = newElement('span');
				span.setAttribute("class", "aboluo-td-a-ban");
				arr[i].style.background = "#ffffff";
				span.innerHTML = holidayList[n].holidayName;
				arr[i].appendChild(span);
			}
		}	
	}
...
function leftrightclick() {
	...
	righta.onclick = function() {
		...
		if (aclick == "") {
			var pervdays1 = getCurrMonthLashDay(yearselect.value,
					monthselect.value + 1);
			if (new Date().getDate() > pervdays1) {
				setRigth(yearselect.value, monthselect.value, pervdays1);
			} else {
				setRigth(yearselect.value, monthselect.value, new Date()
						.getDate());
			}
		} else {
			var adate = aclick.getAttribute("date");
			...
			setRigth(aarr[1] + 1 == 13 ? aarr[0] + 1 : aarr[0],
					aarr[1] + 1 == 13 ? 1 : aarr[1] + 1, aarr[2]);
		}
	}
	lefta.onclick = function() {
		...
		if (aclick == "") {
			// 这个时候向上一个月,那么
			var pervdays1 = getPervMonthLastDay(yearselect.value,
					monthselect.value);
			if (new Date().getDate() > pervdays1) {
				setRigth(yearselect.value, monthselect.value, pervdays1);
			} else {
			...
			setRigth(aarr[1] - 1 == 0 ? aarr[0] - 1 : aarr[0],
					aarr[1] - 1 == 0 ? 12 : aarr[1] - 1, aarr[2]);
		}
	}
...
function aboluoSetrq() {
	var dayType = withID("dayType").value;
	if(dayType == 1){
		//设置日期为festival状态
		var holiday = $("input[name='holiday']")[0].value;
		var begin = $("input[name='begin']")[0].value;
		var end = $("input[name='end']")[0].value;
		if(holiday.length >=6){
			layer.msg('节假日名称过长！', {
			    icon: 5,
			    time: 3000
		...
		var regex = /^\d{1,2}$/;
		if(!regex.test(begin) || begin>24){
			layer.msg('开始日期格式不正确，请输入1-24的整数！', {
			    icon: 5,
			    time: 3000
			}, function(){
				var index = parent.layer.getFrameIndex(window.name);
		...
		if(!regex.test(end) || end>24){
			layer.msg('结束日期格式不正确，请输入1-24的整数！', {
			    icon: 5,
			    time: 3000
			}, function(){
				var index = parent.layer.getFrameIndex(window.name);
		...
		/*
		 * $.ajax({ type:"POST", url: '',
		 * data:{"date":date,"zt":szjjr.childNodes[1].value,"ztid":ztid,"jjrzt":jjrzt},
		 * //这里用ajax可以改变状态 async:false, success:function(json){ if(json.code>0){ var
		 * date=getAclickDomDate(); var datearr=date.split("-");
		 * getjjrszModelByYear(datearr[0]); createTabledate(datearr[0],datearr[1]);
		 * //创建对应的table(日期) setRigth(datearr[0],datearr[1],datearr[2]); //设置右边明细栏内容
		 * }else{ alert("设置失败"); } }, error:function(json){ alert("设置失败"); } });
		 */
		var holidayMaintain = {};
		holidayMaintain.holidayTime = date;
		holidayMaintain.holidayName = holiday;
		holidayMaintain.beginTime = begin;
		holidayMaintain.endTime = end;
		var httpService = new HTTPService(
				"holidayMaintain/saveHolidayMaintain.ajax", holidayMaintain,
				function(data) {
					if (data == "1") {
						layer.msg('保存成功！', {
							icon : 1,
		...
		var httpService = new HTTPService(
				"holidayMaintain/weekDaySet.ajax", parms,
				function(data) {
					if (data == "1") {
						layer.msg('保存成功！', {
							icon : 1,
		...
		parms.date = date;
		var httpService = new HTTPService(
				"holidayMaintain/weekDayOff.ajax", parms,
				function(data) {
					if (data == "1") {
						layer.msg('保存成功！', {
							icon : 1,
...
function delHoliday(){
	...
	var date = curra.getAttribute("date"); // 得到日期
	var querymap = {};
	querymap.holidayTime = date;
	var httpService = new HTTPService(
			"holidayMaintain/delHoliday.ajax", querymap,
			function(data) {
				if(data == 1){
					layer.msg('删除成功！', {
						icon : 1,
...
function queryLunar() {
	var httpService = new HTTPService(
			"holidayMaintain/queryLunar.ajax", null,
			function(data) {
				lunarList = data;
				setB();
			});
